...
The comparison against random sampling is the core test, and it turns on the record budget. Where the
budget is too small for a random draw to span the target system, informed $L_0$ selection should reach
materially lower out-of-sample error, because the gates spend the budget on the records the targets
constrain. Where the budget is generous enough that almost any subset can be reweighted to fit, the two
should converge, and the case for the method then rests on its controls rather than on an accuracy gap.
The reading is therefore one of graceful degradation under aggressive compression rather than a blanket
accuracy win, and the budget at which the methods cross is itself a finding. Reporting
out-of-sample error on held-out families keeps that distinction honest, since a method can always match
the targets it was fit to more tightly than a method that was not.
...
shows how much accuracy must be given up to raise the effective sample size. Whether an interior
setting of the concentration control is acceptable, or whether the fiscal tail simply requires the
concentration, is an empirical question we answer for this target system and not in general.
...
random subset of targets, and it need not flatter any method. Selecting records against one set of
families does not guarantee a sample that reproduces a structurally different family it never saw, and
the family-rotation panel can reverse the ranking on individual folds, including the fold that holds
out the dominant IRS SOI family. We read the rotation as a limitation to be reported per fold rather
than as a single headline number, and widening the held-out surface further is left to future work.
