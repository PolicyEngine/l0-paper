...
We build this pipeline on \microplex{}, a declarative engine for microsimulation dataset
construction. \microplex{} provides the engine and the execution logic, and takes the country-specific content as
a country pack of inputs, so the data sources, the imputation model, the geographic assignment, and
the calibration method are configuration choices rather than fixed code. The paper describes the engine at the level
needed to follow the calibration step, and then describes the configuration used here.
...
Section~\ref{sec:data} describes the base microdata, the \microplex{} pipeline, and the
