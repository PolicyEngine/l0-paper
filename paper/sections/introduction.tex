...
things. First, when does informed $L_0$ selection reproduce the targets more accurately than random
sampling, survey-weight sampling, and the convex $L_1$ alternative at the same record budget, and how
does that comparison change as the budget tightens? Second, does the retained sample generalize, in the sense that it also matches
whole target families held out of the fit? Third, does the framework give usable control over the size
of the output, the geographic level it focuses on, and the concentration of the fitted weights, and
what does tightening the concentration control cost in accuracy? The answers are regime-dependent:
informed selection's accuracy advantage over the baselines concentrates at small budgets, while its
control over size, geography, and weight concentration is what distinguishes it at the larger budgets at
which a dataset is shipped.
