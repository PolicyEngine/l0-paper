...
or its post-selection refit reproduce the targets more accurately than random sampling with reweighting and
survey-weight sampling at the same record budget? Second, what do supplemental raw-error diagnostics reveal
about the tail of the target distribution? Third, does the framework give usable control over the size of
the output and the concentration of the fitted weights? The answer is not the one a method paper would hope
for: on the current full surface, random + reweight is the strongest accuracy baseline. The informed
selector still gives a controlled sparse subset and its post-selection refit improves the raw gated weights,
but the result is a diagnostic frontier rather than a dominance claim.
