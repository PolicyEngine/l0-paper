...
$\lambda_{L_0}$ & $3\times10^{-6}$ to $10^{-4}$ & Sparsity penalty, set per build by bisection to the target size \\
$\lambda_{L_2}$ & \tbc & Soft concentration penalty on $(w_i/w_{0,i})^2$ (mean over records); informed $L_0$ only \\
...
\For{epoch $= 1$ to $E$}
    ...
    \State $\mathcal{L}_{\text{cal}} \gets \frac{1}{m}\sum_{j=1}^{m}\min\!\left(\left|\frac{\hat{t}_j - t_j}{s_j}\right|, c\right)$
