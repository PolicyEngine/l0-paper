...
so any error summary aggregated over all targets is dominated by IRS income-tax aggregates.
...
and scored (Section~\ref{sec:oos}).}
...
sweep. The matched full-surface probe's current runtimes are reported in Section~\ref{sec:results}: dense
...
\begin{figure}[H]
    ...
    predates the fixed normalized-penalty full-surface probe and is used only to motivate reporting runtime
    alongside accuracy.}
