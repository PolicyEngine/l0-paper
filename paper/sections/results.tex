...
objective loss in Equation~\ref{eq:loss_cal}. The run reported here is a matched full-surface probe rather
...
post-$L_0$ refit is essential. The selected support gives a lower finite-budget optimizer loss than
...
rows, the post-$L_0$ refit has a 0.89\% median ARE and an 84.64\% mean ARE. The gap between the median and
the mean is the reason the paper leads with the Populace loss and reports raw ARE only as a diagnostic.

\subsection{Cost}

The matched probe took 39.4 minutes for the $L_0$ selection run. The post-$L_0$ refit then added about
...
concentration controls. Figure~\ref{fig:cost_accuracy} in Appendix~\ref{app:cost} plots wall-clock
runtime against accuracy across requested record budgets as a supplemental cost diagnostic from the
earlier budget sweep.
